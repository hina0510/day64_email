...
public class MailService {
	...
	public void send2() {
		...
		try {
			...
			helper.setTo(to);
			helper.setSubject(subject);
			helper.setText(body, true); //true를 붙이면 html형식으로 전송됨(안붙이면 text형식)
	...
	public void send3(String userId, HttpSession session, String email) {
		...
		String body = "<h2>인증 해주세요</h2>"+"<a href='http://localhost:8081/root/"
				+ "check?userId="+userId+"&userKey="+userKey+"'>인증하기</a>";
		MimeMessage msg = mailSender.createMimeMessage();
		try {
			MimeMessageHelper helper = new MimeMessageHelper(msg, true, "utf-8");
			helper.setTo(email);
